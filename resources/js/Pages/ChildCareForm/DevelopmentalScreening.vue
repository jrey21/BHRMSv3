...
const sortedDevelopmentalScreenings = computed(() => {
    const records = props.child.developmental_screening.map(record => {
        ...
        return {
            ...
            developmental_concerns: record.developmental_concerns,
        };
    });
    console.log(records);
    return records;
...
const showDevelopmentalScreening = ref(true);
...
const addRecord = () => {
    axios.post(`/child/${props.child.id}/addDevelopmentalScreening`, { ...newRecord.value })
        .then(response => {
            ...
            props.child.developmental_screening.push(response.data.record); 
        })
        .catch(error => {
...
const toggleDevelopmentalScreening = () => {
    showDevelopmentalScreening.value = !showDevelopmentalScreening.value;
};
</script>
...
<template>
    ...
    <div v-if="child" class="container">
        ...
        <transition name="fade">
            <div v-if="showDevelopmentalScreening" class="scrollable-table">
                <table class="info-table">
                    <thead>
                        <tr>
                            ...
                            <th>Developmental Concerns</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-if="!child.developmental_screening || child.developmental_screening.length === 0">
                            <td colspan="4" class="no-data" style="text-align: center; font-weight: normal; text-transform: none;">No development screening records available.</td>
                        </tr>
                        <tr v-for="record in sortedDevelopmentalScreenings" :key="record.date">
                            <td>{{ formatDate(record.date) }}</td>
                            <td>{{ record.age + " " + record.age_unit}}</td>
                            <td>{{ record.milestones_achieved }}</td>
                            <td>{{ record.developmental_concerns }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    <transition name="fade">
        <div v-if="showAddModal" class="modal-overlay">
            <div class="modal-content">
                <h2 class="add-vac" style="color: #488a99;">Add Developmental Screening Record</h2>
                <form @submit.prevent="addRecord">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <select name="nit" id="age_unit" v-model="newRecord.age_unit" required style="width: 35%; margin-left: 5%;">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            <label for="milestones_achieved">Milestones Achieved:</label>
                            <input type="text" v-model="newRecord.milestones_achieved" required />
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                            <label for="developmental_concerns">Developmental Concerns:</label>
                            <input type="text" v-model="newRecord.developmental_concerns" required />
                        </div>
                    </div>
