...
const lactatingPeriodData = ref([]);

onMounted(async () => {
    try {
        const response = await axios.get(route('lactating-data'));
        console.log('API Response:', response.data); 
        lactatingPeriodData.value = response.data;
    } catch (error) {
...
const filteredData = computed(() => {
    if (!searchQuery.value) {
        return lactatingPeriodData.value;
    }
    return lactatingPeriodData.value.filter(data =>
...
const totalPages = computed(() => {
    return Math.ceil(lactatingPeriodData.value.length / itemsPerPage);
});
...
const getLatestlactating = (person) => {
    return person.lactating_periods.sort((a, b) => new Date(b.date) - new Date(a.date))[0];
};
...
const downloadPDF = () => {
    ...
    doc.setFontSize(13);
    doc.setFont('helvetica', 'normal');
    doc.text('Lactating Mothers', pageWidth / 2, 55, { align: 'center' });
    ...
    autoTable(doc, {
        ...
        body: sortedData.value.map((data, index) => [
            `${index + 1}.`,
            data.fullName,
            data.lactating_periods.length ? getLatestlactating(data).month : '-' ,
            data.lactating_periods.length ? getLatestlactating(data).weight : '-',
            data.lactating_periods.length ? getLatestlactating(data).height : '-',
            data.lactating_periods.length ? getLatestlactating(data).bmi : '-',
            data.lactating_periods.length ? getLatestlactating(data).status : '-',
            data.lactating_periods.length ? getLatestlactating(data).muac_color : '-',
            data.lactating_periods.length ? getLatestlactating(data).muac_status : '-'
        ]),
    ...
    doc.save('lactating_period_monitoring_data.pdf');
...
<template>
    <Head title=" | lactating Period Monitoring"/>
    <div class="header">
        <h1 class="text-slate-500">Monitoring Form for Nutritional Status of Lactating Mothers</h1>
    </div>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        ...
                        <tr v-for="data in paginatedData" :key="data.id">
                            <td>{{ data.fullName }}</td>
                            <td v-if="data.lactating_periods.length">{{ getLatestlactating(data).month }}</td>
                            <td v-if="data.lactating_periods.length">{{ getLatestlactating(data).weight }}</td>
                            <td v-else>-</td>
                            <td v-if="data.lactating_periods.length">{{ getLatestlactating(data).height }}</td>
                            <td v-else>-</td>
