...
const optimumPractices = ref([]);

onMounted(async () => {
    try {
        const response = await axios.get(route('optimum-practices-data'));
        optimumPractices.value = response.data;
    } catch (error) {
...
const filteredData = computed(() => {
    if (!searchQuery.value) {
        return optimumPractices.value;
    }
    return optimumPractices.value.filter(data =>
...
const totalPages = computed(() => {
    return Math.ceil(optimumPractices.value.length / itemsPerPage);
});
...
<template>
     ...
     <div class="header">
        <h1 class="survey-head text-slate-500">Optimum Practices</h1>
    </div>
